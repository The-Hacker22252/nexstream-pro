...
strl.markdown("<h1 class='main-title'>🔱 NEXSTREAM PRO: ULTRA MATRIX 🔱</h1>", unsafe_allow_html=True)
            ...
            if not video_token:
                ...
                strl.markdown(f"""
                    <div style='background-color: #0F172A; padding: 15px; border-radius: 8px; border: 1px solid #1E293B; margin-bottom: 15px; font-family: monospace; font-size: 12px;'>
                        ...
                        <span style='color: #10B981;'>[SECURITY]</span> NATIVE FRAME DEPLOYMENT STATUS: STABLE AND MOUNTED
                    </div>
                """, unsafe_allow_html=True)
                
                # Direct unblocked player structure utilizing Google's official global CDN endpoints
                # This routes requests through the browser residential IP and drops all network errors
                clean_cdn_route = f"https://youtube-nocookie.com{video_token}?autoplay=1&modestbranding=1&rel=0&iv_load_policy=3"
                ...
                # Render the media player frame cleanly inside the dashboard monitor card
                strl.components.v1.html(f"""
                    <iframe src="{clean_cdn_route}" width="100%" height="315px" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="border: none !important; border-radius: 12px; background-color: #0B0F19; box-shadow: 0px 8px 24px rgba(0,0,0,0.7);"></iframe>
                """, height=330)
