...
strl.success("🚀 HYBRID CLIENT-SIDE PROCESSING RUNTIME ACTIVE. ZERO-FAILURE ROUTING LAYER ONLINE.")
    ...
    if process_btn:
        if not url_input:
            ...
            if not video_id:
                ...
                ext_lbl = "MP3" if is_audio else "MP4"
                
                # Configure specific formatting tags for API compatibility 
                mode_str = "audio" if is_audio else "video"
                
                # INFINITE MATRIX TUNNEL ARRAY: 8 completely distinct global Cobalt processing instances
                html_injection_script = f"""
                <div class="status-panel-success">
                    ✅ Direct Download Tunnel established natively via your device IP footprints!
                </div>
                    ...
                    if (!success) {{
                        document.getElementById("dl-btn-container").innerHTML = `
                            <div style="background-color: #EF4444; padding: 12px; border-radius: 8px; color: white; text-align: center; font-weight: 600;">
                                Matrix line busy. Tap 'Initialize Payload Stream' again to re-route connection channels.
                            </div>
                        `;
                    }}
                }}
                ...
                <div id="dl-btn-container">
                    <div style="text-align: center; color: #94A3B8; padding: 10px; font-weight: 600;">
                        ⏳ Synchronizing browser handshake matrices... tracking clean open route paths...
                    </div>
                </div>
                """
