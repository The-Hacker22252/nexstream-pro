            ...
            if not video_token:
                ...
                strl.markdown(f"""
                    <div style='background-color: #0F172A; padding: 15px; border-radius: 8px; border: 1px solid #1E293B; margin-bottom: 15px; font-family: monospace; font-size: 12px;'>
                        ...
                        <span style='color: #10B981;'>[SECURITY]</span> NATIVE PLAYER INJECTION STATUS: STABLE AND MOUNTED
                    </div>
                """, unsafe_allow_html=True)
                ...
                # RE-ENGINEERED WEB CONTEXT DELIVERY PLAYER:
                # Uses clean HTML5 template logic to display the official unblocked player channel layout.
                # This bypasses the media manager storage layer completely, stopping the MediaFileStorageError code path!
                html_player_module = f"""
                <div style="width: 100%; display: flex; justify-content: center; background-color: #060913; border-radius: 12px; padding: 10px; box-shadow: 0px 8px 24px rgba(0,0,0,0.7);">
                    <iframe src="https://youtube.com{video_token}?autoplay=1&modestbranding=1&rel=0" 
                            width="100%" 
                            height="315px" 
                            frameborder="0" 
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
                            allowfullscreen 
                            style="border-radius: 8px; border: none !important;">
                    </iframe>
                </div>
                """
                strl.components.v1.html(html_player_module, height=340)
